...
class LoginView(QWidget):
    ...
    def init_ui(self):
        ...
        # Admin Hint (Optional: only shows if you start typing admin email)
        self.debug_hint = QLabel("DEBUG MODE DETECTED")
        self.debug_hint.setStyleSheet("color: #FFD700; font-weight: bold; margin-bottom: 5px;")
        self.debug_hint.hide()
        card_layout.addWidget(self.debug_hint)
        self.email_input.textChanged.connect(self._check_debug_status)
        ...
        self.login_btn.setObjectName("PrimaryBtn") # Use your theme's primary button style
    ...
    def _check_debug_status(self, text):
        """Visual cue if the user is entering admin credentials."""
        if text.strip() == self.ADMIN_EMAIL:
            self.debug_hint.show()
        else:
            self.debug_hint.hide()
    ...
    async def handle_login(self):
        ...
        # 🔥 ADMIN BYPASS LOGIC
        if email == self.ADMIN_EMAIL and password == self.ADMIN_PASS:
            print("[DEBUG] Admin login detected. Bypassing Playwright authentication.")
            # We pass None or a "Mock" object. 
            # Ensure your DashboardView/PortfolioView can handle a None client
    ...
    async def perform_authentication(self, email, password):
        ...
        try:
            ...
            if _client_obj:
                ...
                raise Exception("Authentication returned empty session.")
